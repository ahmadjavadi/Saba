...
#endif
    {
        ...
        private void AddToListConstraints(ref List<Constraint> list, SqlDataReader dr)
        {
            ...
            list.Add(new Constraint
            {
                ...
                , ColumnName = string.Format(CultureInfo.InvariantCulture, "[{0}]", dr.GetString(2))
                , UniqueConstraintTableName = uniqueTable
                , UniqueConstraintName = dr.GetString(4)
                , UniqueColumnName = string.Format(CultureInfo.InvariantCulture, "[{0}]", dr.GetString(5))
        ...
        public Int32 GetRowVersionOrdinal(string tableName)
        {
            ...
            {
                int offsetOrdinal = 0;
                object offset = ExecuteScalar("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '" + tableName + "' AND (DATA_TYPE = 'sql_variant' OR DATA_TYPE = 'hierarchyid') AND ORDINAL_POSITION < "+ value.ToString() + ";");
                if (offset != null)
        ...
        public DataTable GetDataFromTable(string tableName, List<Column> tableColumns, List<PrimaryKey> tablePrimaryKeys)
        {
            ...
            sb.AppendFormat(CultureInfo.InvariantCulture, " From [{0}]", GetSchemaAndTableName(tableName));
            ...
            try
            {
                ...
                using (SqlDataAdapter adapter = new SqlDataAdapter())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        {
                            string strSQL = string.Format(CultureInfo.InvariantCulture, "SELECT * FROM [{0}] WHERE 0 = 1", GetSchemaAndTableName(table));
        ...
        private string GetSchemaAndTableName(string table)
        {
            ...
            {
                return (string)ExecuteScalar(string.Format(CultureInfo.InvariantCulture, "SELECT TABLE_SCHEMA FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{0}'", table)) + "].[" + table;
            }
        }
        ...
        {
            return new DateTime(1921,3,21,0,0,0);;
        }
