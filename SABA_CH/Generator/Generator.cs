...
#endif
    {
        ...
        public void ExcludeTables(IList<string> tablesToExclude)
        {
            try
            {
                ...
                catch (NonAcyclicGraphException)
                {
                    ...
                    MessageBox.Show("-- Warning - circular reference preventing proper sorting of tables");
                }
            }
            catch (Exception ex)
            {
                        ...
                        if (fk.ConstraintTableName != fk.UniqueConstraintTableName)
                        {
                            ...
                            if (!schemaDataSet.Relations.Contains(fk.ConstraintName) && fkColumns != null)
                            {
                                ...
                                catch (ArgumentException ex1)
                                {
                                    ...
                                    MessageBox.Show("-- Warning - constraint: " + fk.ConstraintTableName + " " + ex1.Message);

                                }
                                catch (InvalidConstraintException ex2)
                                {
                                    _sbScript.AppendLine("-- Warning - constraint: " + fk.ConstraintTableName + " " + ex2.Message);
                                    MessageBox.Show("-- Warning - constraint: " + fk.ConstraintTableName + " " + ex2.Message);
                                }
                            }
                        }
                    }
        ...
        public void GenerateTableContent(string tableName, bool saveImageFiles, bool ignoreIdentity = false)
        {
            try
            {
                ...
                using (IDataReader rdr = _repository.GetDataFromReader(tableName, columns, _allPrimaryKeys.Where(p => p.TableName == tableName).ToList()))
                {
                    bool firstRun = true;
                    int rowCount = 0;
                    ...
                    while (rdr.Read())
                    {
                        ...
                        for (int iColumn = 0; iColumn < rdr.FieldCount; iColumn++)
                        {
                            ...
                            else if (rdr.GetFieldType(iColumn) == typeof(DateTime))
                            {
                                ...
                                switch (format)
                                {
                                    case DateFormat.None:
                                        ...
                                        _sbScript.Append(date.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
                                    ...
                                    case DateFormat.Date:
                                        ...
                                        _sbScript.Append(date.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture));
                                        _sbScript.Append("'");
                                        break;
                                    case DateFormat.DateTime2:
                                        _sbScript.Append(unicodePrefix + "'");
                                        _sbScript.Append(date.ToString("yyyy-MM-dd HH:mm:ss.fffffff", CultureInfo.InvariantCulture));
                                        _sbScript.Append("'");
                                        break;
                                }
                            ...
                            {
                                string intString = Convert.ToString(rdr.GetValue(iColumn), CultureInfo.InvariantCulture);
                                _sbScript.Append(intString);
                            }
        ...
        private string GenerateInserOrUpdate(string tableName, bool createInsert, DataRow row)
        {
            ...
            try
            {
                ...
                for (int iColumn = 0; iColumn < row.ItemArray.Count(); iColumn++)
                {
                    ...
                    {
                        string intString = NumericConverter.DoubleConverter(row[iColumn].ToString()).ToString("R", CultureInfo.InvariantCulture);
                        sb.Append(intString);
                    }
        ...
        public void GenerateTableInsert(string tableName, IList<string> fields, IList<string> values, int lineNumber)
        {
            try
            {
                if (fields.Count != values.Count)
                {
                    ...
                    throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Error on line {0} in the csv file. The number of values ({1}) and fields ({2}) do not match - {3}", lineNumber, values.Count, fields.Count, valueString.ToString()));
                }
                List<Column> columns = _allColumns.Where(c => c.TableName == tableName).ToList();
                if (columns.Count > 0)
                {
        ...
        public string SqlFormatValue(string tableName, string fieldName, string value)
        {
            ...
            try
            {
                ...
                else if (column.DataType == "datetime")
                {
                    ...
                    //Datetime globalization - ODBC escape: {ts '2004-03-29 19:21:00'}
                    _sbScript.Append("{ts '");
                    _sbScript.Append(date.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture));
                    _sbScript.Append("'}");
                }
        ...
        private void Init(IRepository repository, string outFile)
        {
            try
            {
                ...
                //_sbScript = new StringBuilder(548,1000);
            ...
            try
            {
                ...
                foreach (var tableName in _tableNames)
                {
                    ...
                    foreach (var column in columns)
                    {
                        ...
                        if (clrType.FullName == "System.String" && column.CharacterMaxLength > 0)
                        {
                        ...
                        indentWriter.WriteLine("public " + clrType.Name + "  " + column.ColumnName + " { get; set; }");
                        indentWriter.WriteLine("");
                    }
                    indentWriter.Indent = 1;
                    indentWriter.WriteLine("}");
