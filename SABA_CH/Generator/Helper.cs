    ...
    public static class Helper
    {
        public static string FinalFiles
        {
            get
            {
                ...
                {
                    if (finalFiles.EndsWith(", ", StringComparison.OrdinalIgnoreCase))
                    {
        ...
        public static void WriteIntoFile(string script, string fileLocation, int increment, bool sqlite)
        {
            if (increment > -1)
            {
                ...
                string ext = Path.GetExtension(Path.GetFileName(fileLocation));
                string path = Path.GetDirectoryName(fileLocation);
                string name = Path.GetFileNameWithoutExtension(fileLocation);
                fileLocation = Path.Combine(path, name + "_" + increment.ToString("0000" + CultureInfo.InvariantCulture));
            ...
            try
            {
                ...
                Encoding encoder = new UTF8Encoding(false);
                if (!sqlite)
        ...
        {
            return connectionString.Replace(string.Format(CultureInfo.InvariantCulture, ";Timeout = \"{0}\"", timeout), string.Empty);
        }
        ...
        internal static string CheckDataType(string dataType, Column col, bool refToIdentity, bool preserveDateAndDateTime2)
        {
            switch (dataType)
            {
                ...
                case "time":
                    return "nvarchar";
                default:
                    throw new NotSupportedException(string.Format(CultureInfo.InvariantCulture, "Data type {0} in table {1}, colum {2} is not supported, please change to a supported type", dataType, col.TableName, col.ColumnName));
            }
        }
            ...
            {
                {
                    ...
                    {
                        var pks = primaryKeys.Where(c => c.KeyName.Equals(value, StringComparison.Ordinal) && c.TableName == tableName).ToList();
                        if (pks.Count > 0)
                        {
        ...
        public static string ShowErrors(SqlException e)
        {
        ...
        public static string ShowErrors(SqlCeException e)
        {
            StringBuilder bld = new StringBuilder();
            Exception inner = e.InnerException;
            ...
            if (e.Errors != null)
            {
                ...
                {
                    bld.Append("\n Error Code: " + err.HResult.ToString("X", CultureInfo.InvariantCulture));
                    bld.Append("\n Message   : " + err.Message);
