...
namespace SABA_CH.UI
{
    ...
    public partial class TimeControl : UserControl
    {
        public TimeControl()
        {
            ...
            txtSecound.Text = DateTime.Now.Second.ToString();
        }
        ...
        public DateTime? DateTimeValue
        {
            get
            {
                ...
                string secound = this.txtSecound.Text;
                if (!string.IsNullOrWhiteSpace(hours)
                    && !string.IsNullOrWhiteSpace(minutes)
                    && !string.IsNullOrWhiteSpace(secound))
                {
                    string value = string.Format("{0}:{1}:{2}", this.txtHours.Text, this.txtMinutes.Text, this.txtSecound.Text);
        ...
        private void btnDown_Click(object sender, RoutedEventArgs e)
        {
            ...
            if ("txtMinutes".Equals(controlId))
            {
                this.ChangeMinutes(false);
            }
            else if ("txtSecound".Equals(controlId))
            {
                this.ChangeSecound(false);
            }
            else 
            {
        ...
        /// <summary>
        /// Handles the PreviewTextInput event of the txtSecound control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="System.Windows.Input.TextCompositionEventArgs"/> instance containing the event data.</param>
        private void txtSecound_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
        ...
        private void txt_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            ...
            else if ("txtSecound".Equals(textBox.Name) && value > 59)
            {
                EnforceLimits(e, textBox);
            }
        }
        ...
        /// </summary>
        /// <param name="isUp">if set to <c>true</c> [is up].</param>
        private void ChangeSecound(bool isUp)
        {
            int value = Convert.ToInt32(this.txtSecound.Text);
            if (isUp)
            {
                value += 1;
                if (value == 60)
            ...
            this.txtSecound.Text = textValue;
        }
        ...
        private TextBox GetControlWithFocus()
        {
            ...
            else if (this.txtSecound.IsFocused)
            {
                txt = this.txtSecound;
            }
            return txt;
