...
namespace ErikEJ.SqlCeScripting
{
    ...
    public class Column
    {
        ...
        public string ShortType
        {
...

            {
                ...
                {
                    return string.Format(CultureInfo.InvariantCulture, "{0}({1}),", this.DataType, this.CharacterMaxLength);
                }
                else if (this.DataType == "numeric")
                {
                    return string.Format(CultureInfo.InvariantCulture, "{0}({1},{2}),", this.DataType, this.NumericPrecision, this.NumericScale);
                }
                else
                {
                    return string.Format(CultureInfo.InvariantCulture, "{0},", this.DataType);
                }
            }
        }
    }
    ...
    class ColumnComparer : IEqualityComparer<Column>
    {
        ...
        public int GetHashCode(Column column)
        {
            ...
            if (ReferenceEquals(column, null)) return 0;
