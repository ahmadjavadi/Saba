...
{
    public class Meter_Consumption_Data
    {
        ...
        internal void VerifyData()
        {
            ...
            try
            {
                ...
                if (Valid_Monthly_Consumption_Data_List == null)
                    validData = false;
                Verify_Consumption_Data verify_Consumption_Data = new Verify_Consumption_Data(OrginalWaterDataList);
                ...
                if (WaterData && Non_Valid_WaterDataList.Count > 0) // New Reading Water Data Exist
                {
                    ...
                    if (!validData || Valid_Monthly_Consumption_Data_List.Count == 0)   //  درصورتیکه داده معتبر نداشته باشیم؟
                    {
                        ...
                        {
                            Valid_Monthly_Consumption_Data_List = verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Non_Valid_WaterDataList[Non_Valid_WaterDataList.Count - 1]);

                        }
                        else if (Non_Valid_WaterDataList.Count == 1)// فقط یک قرائت داریم
                        {
                            Valid_Monthly_Consumption_Data_List = Non_Valid_WaterDataList;
                            verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Non_Valid_WaterDataList[Non_Valid_WaterDataList.Count - 1]);
                        }
                    }
                    ...
                    else
                    {
                        ...
                        Non_Valid_WaterDataList = verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Valid_Monthly_Consumption_Data_List[0]);
                        Valid_Monthly_Consumption_Data_List.InsertRange(0, verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Valid_Monthly_Consumption_Data_List[0]));
                    }
                }
            ...
            try
            {
                ...
                if (WaterData && Non_Valid_WaterDataList.Count > 0) // New Reading Water Data Exist
                {
                    ...
                    if (!validData || Valid_Monthly_Consumption_Data_List.Count == 0)   //  درصورتیکه داده معتبر نداشته باشیم؟
                    {
                        ...
                        if (Non_Valid_WaterDataList.Count > 1)
                        {
                            Valid_Monthly_Consumption_Data_List = verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Non_Valid_WaterDataList[Non_Valid_WaterDataList.Count - 1],softwareVersion);

                        }
                        else if (Non_Valid_WaterDataList.Count == 1)// فقط یک قرائت داریم
                        {                            
                            Valid_Monthly_Consumption_Data_List = Non_Valid_WaterDataList;
                            verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Non_Valid_WaterDataList[Non_Valid_WaterDataList.Count - 1], softwareVersion);
                        }
                    }
                    ...
                    {
                        // داده های جدید باید اعتبارسنجی شده و ذخیره شوند 
                        Non_Valid_WaterDataList = verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Valid_Monthly_Consumption_Data_List[0], softwareVersion);
                        Valid_Monthly_Consumption_Data_List.InsertRange(0, verify_Consumption_Data.Data_Correction(Non_Valid_WaterDataList, (Water_Data_Reading_Class)Valid_Monthly_Consumption_Data_List[0]));                        
                    }
                }
                ...
                for (int counter = 0; counter < Valid_Monthly_Consumption_Data_List.Count; counter++)
                {
                    try
                    {
                        ...
                        Water_Data_Reading_Class wdrc = new Water_Data_Reading_Class(Valid_Monthly_Consumption_Data_List[counter].Reading_Date,
                            Valid_Monthly_Consumption_Data_List[counter].Error, Valid_Monthly_Consumption_Data_List[counter].TotalWater, meterNo, Valid_Monthly_Consumption_Data_List[counter].TotalWater);
