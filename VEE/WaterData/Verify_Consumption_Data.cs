...
namespace VEE.WaterData
{
    class Verify_Consumption_Data
    {
        ...
        public Verify_Consumption_Data(List<Water_Data_Reading_Class> OrginalWaterDataList)
        {
        ...
        public List<Water_Data_Reading_Class> Data_Correction(List<Water_Data_Reading_Class> Water_Data_Reading_List, Water_Data_Reading_Class objValidData, string softwareversion = "")
        {
            ...
            if((CheckWaterConsumption(objValidData)) &&  ( (Water_Data_Reading_List.Count > 0 && Meter_Consumption_Data.ConsumedWaterdata.Count == 0 && !IsZeroConsumption(objValidData))))
            ...
            for (_listTransversal = Water_Data_Reading_List.Count - 1; _listTransversal > 0; _listTransversal--)
            {
                ...
                try
                {
                    if (VerifyDate(currentReading.Reading_Date))
                    {
                        ...
                        //for (int j = 1; j < previousReading.water_data_Reading_List.Count; j++)
                        //{

                        //    if (Convert.ToDecimal(previousReading.water_data_Reading_List[j].Water_Consumption) != 0)
                        //    {
                        //        isZeroData = false;
                        //        break;
                        //    }
                        ...
                        //    InsertDataIntoConsmedWater(currentReading, 22);
                        //}
                        //else

                        if (!CheckWaterConsumption(currentReading))
                        {
                            continue;
                        }
                        if ((Water_Data_Reading_List.Count > 0 && Meter_Consumption_Data.ConsumedWaterdata.Count == 0 && !IsZeroConsumption(currentReading)))
                            InsertDataIntoConsmedWater(currentReading, 22, softwareversion);
                        else
                        {
                            ...
                            if (temp != null)
                            {
                                ...
                                InsertLastConsumptionToConsumedWater(currentReading);
                        }
                    }
                }
                catch (Exception ex)
        ...
        private bool CheckWaterConsumption(Water_Data_Reading_Class reading_Class)
        {
            try
            {
                if (Convert.ToDecimal(reading_Class.TotalWater) > 0)
                {
                    var ta = Convert.ToDecimal(reading_Class.TotalWater);
                    var t0 = Convert.ToDecimal(reading_Class.water_data_Reading_List[0].Water_Consumption);
                    if (ta - t0 > 0)
                        return true;
                    return false;
                }
        ...
        private void InsertLastConsumptionToConsumedWater(Water_Data_Reading_Class currentReading)
        {
            var date1 = GetConsumedDate(currentReading.Reading_Date, 1);
            var totalconsumption = Convert.ToDouble(currentReading.TotalWater) - currentReading.water_data_Reading_List[0].Water_Consumption;
            if (!Meter_Consumption_Data.ConsumedWaterdata.Any(x => x.ConsumedDate.Contains(date1)))
            {
                Meter_Consumption_Data.ConsumedWaterdata.Insert(0, new VeeConsumedWater()
                {
                    CustomerId = null,
                    MeterId = Meter_Consumption_Data.MeterId,
                    ConsumedDate = date1,
                    Flow = currentReading.water_data_Reading_List[1].Month_Maximum_Debi.ToString(),
                    MonthlyConsumption = currentReading.water_data_Reading_List[1].Water_Consumption.ToString(),

                    TotalConsumption = totalconsumption.ToString(),
                    IsValid = currentReading.water_data_Reading_List[1].IsValidData
                });
            }
        }
        ...
        private void InsertDataIntoConsmedWater(Water_Data_Reading_Class temp, int monthDiff, string softwareVersion)
        {
            ...
            decimal totalWater = Convert.ToDecimal(temp.TotalWater);
            totalConsumption[0] = totalWater;
            monthlyConsumption[0] = Convert.ToDecimal(temp.water_data_Reading_List[0].Water_Consumption);
            ...
            {
                if (!Meter_Consumption_Data.ConsumedWaterdata.Any(x => x.ConsumedDate.Contains(temp.Reading_Date)))
                {
                    Meter_Consumption_Data.ConsumedWaterdata.Insert(0, new VeeConsumedWater()
                    {
                        CustomerId = null,
                        ...
                        TotalConsumption = totalConsumption[0].ToString(),
                        IsValid = true
                    });
                }
                return;
            ...
            for (int j = 1; j < temp.water_data_Reading_List.Count; j++)
            {
                if (j > 23)
                    break;
                if (temp.water_data_Reading_List[j - 1].Water_Consumption == -1)
                    continue;
                totalWater -= Convert.ToDecimal(temp.water_data_Reading_List[j - 1].Water_Consumption);
                totalConsumption[j] = totalWater;
                monthlyConsumption[j] = Convert.ToDecimal(temp.water_data_Reading_List[j].Water_Consumption);
            }
            if (monthDiff < 24)
                for (int j = monthDiff + 1; j > 0; j--)
                {
            ...
            else
            {
                ...
                for (int j = monthlyConsumption.Length - 1; j > 0; j--)
                {
                    ...
                    try
                    {
                        ...
                        //totalWaater = totaltemp[j].water_data_Reading_List[j].Water_Consumption.ToString();
                        if (!Meter_Consumption_Data.ConsumedWaterdata.Any(x => x.ConsumedDate.Contains(date2)))
                        {
                            Meter_Consumption_Data.ConsumedWaterdata.Insert(0, new VeeConsumedWater()
                            {
                                CustomerId = null,
        ...
        string GetConsumedDate(string readDate, int index)
        {
            ...
            return $"{year}/{newMonth.ToString("00")}/01";
        ...
        private Water_Data_Reading_Class Water_Data_verification(Water_Data_Reading_Class previousReading, Water_Data_Reading_Class currentReading, out int Month_Diff)
        {
            ...
            if (previousReading.TotalWater == currentReading.TotalWater)
            {
                // کنترر خاموش بوده و هیچ کپچری هم ثبت نشده است
                if (previousReading.water_data_Reading_List[0].Water_Consumption == currentReading.water_data_Reading_List[0].Water_Consumption
                    && previousReading.water_data_Reading_List[1].Water_Consumption == currentReading.water_data_Reading_List[1].Water_Consumption
                    )
                {
                    //System.IO.File.AppendAllText("d://samedatameetrs",temp.MeterNo);
                    // کنتور خاموش بوده است بنابراین به ازای هر ماه خاموشی صفر لحاظ شود
                    for (int i = 0; i < Month_Diff; i++)
                    {
                        temp.water_data_Reading_List.Insert(1, new Water_data("0", "0", false));
                    }
                    return temp;
            ...
            if (IsZeroConsumption(previousReading))
            {
            ...
            if (nearest_Valid_Water_Data.IsValidData || previousReading.Error == "-3")
            {
                ...
                if (newValidDataIndex > -1)
                {
                    ...
                    //if (oldValidDataIndex > 1 && jumpCount > 0)
                    //{
                    //    System.IO.File.AppendAllText("D:\\Meetrs.txt", currentReading.MeterNo + "\r\n");
                    //}
                    newValidDataIndex = newValidDataIndex - oldValidDataIndex + 1;
                    oldValidDataIndex = 1;
                    if (jumpCount < 0)
                    {
                        //SaveToExcell(temp, previousReading, "jump lessthan 0");
                        //currentReading.Error = "  خطا: احتمالا تاریخ کنتور تغییر پیدا کرده است یا اینکه کنتور خاموش بوده است.";
                        // LogFile.WriteLOG(serial + " ( " + currentReading.Reading_Date + " ): " + currentReading.Error,false);
                        return temp;
                    }
                    ...
                    {
                        int indexWd = 1;
                        var wd = temp.water_data_Reading_List[1];
                        temp.Error = "خطا: احتمالا تاریخ کنتور تغییر پیدا کرده است";
                        if (jumpCount == 1)
                        {
                            if (newValidDataIndex < 3)
                                newValidDataIndex = newValidDataIndex;

                            // فقط یک داده اضافی داریم
                            for (int i = 2; i < newValidDataIndex - 1; i++)
                            {
                                if (wd.Water_Consumption > temp.water_data_Reading_List[i].Water_Consumption)
                                {
                                    wd = temp.water_data_Reading_List[i];
                                    indexWd = i;
                                }
                            }
                            if (temp.water_data_Reading_List[indexWd - 1].Water_Consumption > temp.water_data_Reading_List[indexWd + 1].Water_Consumption)
                            {
                                wd.Water_Consumption += temp.water_data_Reading_List[indexWd + 1].Water_Consumption;
                                wd.Month_Maximum_Debi = wd.Month_Maximum_Debi > temp.water_data_Reading_List[indexWd + 1].Month_Maximum_Debi ? wd.Month_Maximum_Debi : temp.water_data_Reading_List[indexWd + 1].Month_Maximum_Debi;
                                temp.water_data_Reading_List[indexWd] = wd;
                                temp.water_data_Reading_List.RemoveAt(indexWd + 1);
                            }
                            else
                            {
                                wd.Water_Consumption += temp.water_data_Reading_List[indexWd - 1].Water_Consumption;
                        ...
                        if (jumpCount > 1)
                        {
                            temp = RemoveJump5(previousReading, temp,ref newValidDataIndex, oldValidDataIndex, jumpCount - 1);
                            wd.Water_Consumption = temp.water_data_Reading_List[1].Water_Consumption;
                            for (int i = 2; i < newValidDataIndex - 1; i++)
                            {
                                if (i == temp.water_data_Reading_List.Count) break;
                                if (wd.Water_Consumption > temp.water_data_Reading_List[i].Water_Consumption)
                                {
                                    wd = temp.water_data_Reading_List[i];
                                    indexWd = i;
                        ...
                        RemoveJump3(temp, wd, indexWd);
                        return temp;
                        //SaveToExcell(currentReading,previousReading, "jump persend 2 ==1  ");
                        // LogFile.WriteLOG(serial + " ( " + currentReading.Reading_Date + " ): " + currentReading.Error,false);
                    }
                    ...
                    if (jumpCount > 1)
                    {
                        //SaveToExcell(temp, previousReading, " has jump   ");
                        temp = RemoveJump5(previousReading, temp,ref newValidDataIndex, oldValidDataIndex, jumpCount);
                    }
                    #endregion
        ...
        private bool IsZeroConsumption(Water_Data_Reading_Class reading)
        {
            if (Convert.ToDecimal(reading.TotalWater) == 0)
                return true;
            for (int i = 1; i < reading.water_data_Reading_List.Count; i++)
            {
                if (reading.water_data_Reading_List[i].Water_Consumption > 0)
                    return false;
            }
            return true;
        }

        private void RemoveJump3(Water_Data_Reading_Class temp, Water_data wd, int newValidDataIndex)
        {
            if (newValidDataIndex == 1)
            {
            ...
            {
                if (temp.water_data_Reading_List[newValidDataIndex + 1].Water_Consumption > temp.water_data_Reading_List[newValidDataIndex - 1].Water_Consumption)
                {
                    Water_data wdnew = new Water_data((temp.water_data_Reading_List[newValidDataIndex - 1].Water_Consumption + wd.Water_Consumption).ToString(),
                       temp.water_data_Reading_List[newValidDataIndex - 1].Month_Maximum_Debi.ToString(), false);

                    RemoveJumpFromAllReadingData(temp.water_data_Reading_List[newValidDataIndex - 1], temp.water_data_Reading_List[newValidDataIndex], null, wdnew);


                    temp.water_data_Reading_List[newValidDataIndex - 1].Water_Consumption += wd.Water_Consumption;
                    temp.water_data_Reading_List[newValidDataIndex - 1].IsValidData = false;
                    temp.water_data_Reading_List.RemoveAt(newValidDataIndex);
                }
                else
                {
                    Water_data wdnew = new Water_data((temp.water_data_Reading_List[newValidDataIndex + 1].Water_Consumption + wd.Water_Consumption).ToString(),
                    ...
                    RemoveJumpFromAllReadingData(null, temp.water_data_Reading_List[newValidDataIndex], temp.water_data_Reading_List[newValidDataIndex - 1], wdnew);
                    temp.water_data_Reading_List[newValidDataIndex + 1].Water_Consumption += wd.Water_Consumption;
                    temp.water_data_Reading_List[newValidDataIndex + 1].IsValidData = false;
                    temp.water_data_Reading_List.RemoveAt(newValidDataIndex);
        ...
        private Water_Data_Reading_Class RemoveJump5(Water_Data_Reading_Class previousReading, Water_Data_Reading_Class currentReading,ref int newValidDataIndex, int oldValidDataIndex, int jumpCount)
        {
            ...
            {
                RemoveJump6(ref currentReading, previousReading, false,ref newValidDataIndex, JumpNumber, oldValidDataIndex);
            }
            #endregion
            ...
            {
                RemoveJump7(ref currentReading, previousReading, false, newValidDataIndex, JumpNumber, oldValidDataIndex, number_of_InValid_Data_withOut_Zero);
            }
            #endregion
            ...
            {
                RemoveJump6(ref currentReading, previousReading, true,ref newValidDataIndex, JumpNumber, oldValidDataIndex);
            }
            #endregion
            ...
            {
                var v1=     JustRemoveJump(ref currentReading, previousReading, false, newValidDataIndex,ref JumpNumber, oldValidDataIndex);
                if(JumpNumber>0)
                    newValidDataIndex=     Remove_Zero_Consumption(ref currentReading, previousReading, v1, JumpNumber - number_of_InValid_Data_withOut_Zero,  oldValidDataIndex);
            ...
            else
            {
                ...
                MergeLowerConsumption(ref currentReading, previousReading,ref newValidDataIndex, JumpNumber, oldValidDataIndex);
        ...
        private void MergeLowerConsumption(ref Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading,ref int newValidDataIndex, int jumpNumber, int oldValidDataIndex)
        {
            try
            {
                ...
                while (jumpNumber > 0)
                {
                    ...
                    // فعلا حذفش میکنم تا ببینم فیدبک چیه
                    if (!HighLow(wd))
                    {
                        //break;
                        //System.IO.File.AppendAllText("D:\\HighLow_Error.txt", "\r\n" + currentReading.MeterNo + " " + currentReading.Reading_Date);
                    }
        ...
        private int Find_Valid_Water_Data_In_CurrentReading(Water_data valid_Water_Data, Water_Data_Reading_Class currentReading)
        {
            for (int i = 0; i < currentReading.water_data_Reading_List.Count; i++)
            {
                if (currentReading.water_data_Reading_List[i].Water_Consumption == valid_Water_Data.Water_Consumption &&
                       currentReading.water_data_Reading_List[i].Month_Maximum_Debi == valid_Water_Data.Month_Maximum_Debi)
                    return i;
            }
            return -1;
        }
        ...
        {
            int cnt = 0;
            for (int i = 1; i < oldValidDataIndex; i++)
            {
        ...
        private Water_Data_Reading_Class Modified_Data_With_Large_Number_Of_Jumps(Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading)
        {
            ...
            return RemoveJump4(currentReading, previousReading, Month_diff, first_unvalid_data_Position);
        }
        ...
        private Water_Data_Reading_Class verification_With_orginal_Previous_Reading(Water_Data_Reading_Class previousReading, Water_Data_Reading_Class currentReading)
        {
            ...
            if (nearest_Valid_Water_Data.IsValidData)
            {
                ...
                {
                    // تعداد پرشها خیلی زیاده به نظر شاید بشه با حذف همه اطلاعات حداقل از قرائت های جدید  استفاده بهتری کرد
                    //System.IO.File.AppendAllText("D:\\ MeterError.txt", "\r\n" + currentReading.MeterNo + " " + currentReading.Reading_Date);
                    return null;
                }
                ...
                if (valid_WaterDataPosition_In_Current_Reading > -1)
                {
                    ...
                    if (jumpCount > 1)
                    {
                        ...
                        if (JumpNumber <= number_of_InValid_Data_withOut_Zero)
                        {
                            RemoveJump6(ref currentReading, previousReading, false,ref valid_WaterDataPosition_In_Current_Reading, JumpNumber, valid_WaterDataPosition_In_Previous_Reading);
                        }
                        #endregion
                        ...
                        {
                            RemoveJump6(ref currentReading, previousReading, true,ref valid_WaterDataPosition_In_Current_Reading, JumpNumber, valid_WaterDataPosition_In_Previous_Reading);
                        }
                        #endregion
                    ...
                    else
                    {
                        ...
                        for (int i = 1;
                            valid_WaterDataPosition_In_Current_Reading + i < 24; i++)
                        {
        ...
        private Water_Data_Reading_Class Water_Data_Verificaiton(Water_Data_Reading_Class currentReading, int MonthDiff)
        {
            ...
            if (sum == (double.Parse(currentReading.TotalWater, System.Globalization.NumberStyles.AllowDecimalPoint, System.Globalization.NumberFormatInfo.InvariantInfo)))
                g = 0;
            for (int m = currentReading.water_data_Reading_List.Count; currentReading.water_data_Reading_List.Count < 24; m++)
            {
                Water_data wd = new Water_data();
                wd.IsValidData = false;
        ...
        private void RemoveJump6(ref Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, bool considerZero,ref
            int valid_WaterDataPosition_In_Current_Reading, int JumpNumber, int valid_WaterDataPosition_In_Previous_Reading)
        {
            for (int y = 2; y < valid_WaterDataPosition_In_Current_Reading; y++)
            {
                if (Compute_Rate((Water_data)currentReading.water_data_Reading_List[y], considerZero))
                {
                    if (Compute_Month_Number(currentReading.Reading_Date, y) < 8)
                    {
                        ...
                        else
                        {
                            ...
                            y = 1;
                        valid_WaterDataPosition_In_Current_Reading -= 2;
                        JumpNumber--;
                        if (JumpNumber == 0)
                            break;
                    }
            ...
            if (JumpNumber > 0)
            {
                MergeLowerConsumption(ref currentReading, previousReading,ref valid_WaterDataPosition_In_Current_Reading, JumpNumber, valid_WaterDataPosition_In_Previous_Reading);
            }
            int cPosition = valid_WaterDataPosition_In_Current_Reading - (24 - currentReading.water_data_Reading_List.Count);

            //for (int i = 1; currentReading.water_data_Reading_List.Count < 24; i++)
            //{
        ...
        private int JustRemoveJump(ref Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, bool considerZero,
            int valid_WaterDataPosition_In_Current_Reading,ref int JumpNumber, int valid_WaterDataPosition_In_Previous_Reading)
        {
            var counter = valid_WaterDataPosition_In_Current_Reading;
            ...
            if (JumpNumber > 0)
            {
                for (int y = 1; y < valid_WaterDataPosition_In_Current_Reading; y++)
                {
                    if (y >= currentReading.water_data_Reading_List.Count)
                        break;
                    if (Compute_Rate((Water_data)currentReading.water_data_Reading_List[y], considerZero))
            ...
            return valid_WaterDataPosition_In_Current_Reading;
        }
        private void RemoveJump7(ref Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, bool considerZero,
        ...
        private Water_Data_Reading_Class RemoveJump4(Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, int Month_diff, int first_unvalid_data_Position)
        {
            int JumpNumber = (24 - Month_diff) / 2;
            ...
            if (JumpNumber > 1)
            {
                    ...
                    + unSaved_Consumption) * 1000 / maxDebi < Time_Diff)
                {
                        ...
                        unSaved_Consumption) <= 1.2 * maxConsume)
                    {
                        ...
                        while (tempReading.water_data_Reading_List.Count < 24)
                        {
                            Water_data wd = new Water_data();
                            wd.IsValidData = false;
                            tempReading.water_data_Reading_List.Add(wd);
        ...
        private Water_Data_Reading_Class RemoveAllUnvalidData(Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, int Month_Diff, int first_unvalid_data_Position)
        {
            ...
            RemoveJump(ref currentReading, previousReading, true, 24);
            ...
            while (currentReading.water_data_Reading_List.Count < 24)
            {
                Water_data wd = new Water_data();
                wd.IsValidData = false;
                currentReading.water_data_Reading_List.Add(wd);
            ...
            for (int m = first_unvalid_data_Position; m < 24; m++)
            {
        ...
        private void Insert_PreviousReading_to_currentReading(Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, int valid_WaterDataPosition_In_Previous_Reading, int position)
        {
            ...
            if (index_In_Orginal_Previous_Reading < 24)
            {
                int k = 1;
                for (int i = position - index_In_Orginal_Previous_Reading + 1; i < 24; i++)
                {
                    currentReading.water_data_Reading_List[i] = previousReading.water_data_Reading_List[k];
                    k++;
                    if (k > 23)
        ...
        private int Remove_Zero_Consumption(ref Water_Data_Reading_Class currentReading, Water_Data_Reading_Class previousReading, int valid_WaterDataPosition_In_Current_Reading, int JumpNumber, int valid_WaterDataPosition_In_Previous_Reading)
        {
            try
            {
                var counter = valid_WaterDataPosition_In_Current_Reading;
                for (int y = 2; y < counter - 2; y++)
                {
        ...
        private void Remove_All_Zero_Consumption(Water_Data_Reading_Class currentReading)
        {
            try
            {
                for (int y = 2; y < currentReading.water_data_Reading_List.Count - 2; y++)
                {
                    if (Compute_Rate((Water_data)currentReading.water_data_Reading_List[y], true))
                    {
                        Water_data wd = new Water_data();
        ...
        private int Find_Nearest_Valid_Water_Data(Water_Data_Reading_Class previousReading, int Index_Of_Valid_Zero, ref Water_data nearest_Valid_Water_Data)
        {
            ...
            int valid_WaterDataPosition_In_previousReading = 1;
        ...
        private int DiffOfShamsiDate(string previousDate, string currentDate)
        {
            ...
            try
            {
                ...
                if (currentDateTime.Year > previousDateTime.Year)
                    dif = 12 * (currentDateTime.Year - previousDateTime.Year - 1) + (12 - previousDateTime.Month) + currentDateTime.Month;
                else
                    dif = currentDateTime.Month - previousDateTime.Month;
        ...
        private int Search_Nearest_Common_Valid_Data(Water_data valid_water_Data, int valid_data_Position, Water_Data_Reading_Class currentReading, int MonthDiff, ref int position, Water_Data_Reading_Class previousReading)
        {
            ...
            for (currentCounter = MonthDiff + valid_data_Position; currentCounter < currentReadingConsumedData.Count; currentCounter++)
            {
                if (valid_water_Data.Water_Consumption == ((Water_data)currentReadingConsumedData[currentCounter]).Water_Consumption &&
                    valid_water_Data.Month_Maximum_Debi == ((Water_data)currentReadingConsumedData[currentCounter]).Month_Maximum_Debi)
                    return currentCounter;
            }
            if (currentCounter == 24 && valid_water_Data.Water_Consumption > 0)
            {
                ...
                for (pos = 0; pos < MonthDiff + valid_data_Position; pos++)
                {
                   ...
                   valid_water_Data.Month_Maximum_Debi == ((Water_data)currentReadingConsumedData[pos]).Month_Maximum_Debi)
                    {
                        position = pos;
                        break;
                    }
            ...
            if (position == -1) // داده قرائت قبلی در قرائت جدید وجود ندارد
            {
                return -1;
            }
            if (pos == -1)
            ...
            if (MonthDiff + valid_data_Position > position)
            {
                ...
                for (int i = 0; i < MonthDiff + valid_data_Position - position; i++)
                {
                    if (position == 1)
                    {
                        currentReadingConsumedData.Insert(position, new Water_data("0", "0", false));
                    }
                    else
                        currentReadingConsumedData.Insert(2, new Water_data("0", "0", false));
